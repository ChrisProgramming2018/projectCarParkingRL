...
def attention(q, k, v, d_k, mask=None, dropout=None):
    ...
    print("")
    print("")
    print("attention q ", q.shape)
    print("attention k ", k.shape)
...
class TX(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        # q: (b , dim )
        print("TX input ", q.shape)
        print("TX input ", k.shape)
        print("TX input ", v.shape)
        b = q.size(0)
        t = k.size(1)
        dim = q.size(1)
        ...
        print("new query ", new_query.shape)
        return new_query
...
class Block_head(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        self.count +=1
        print("Block head q ", q.shape)
        print("Block head k ", k.shape)
        print("Block head v ", v.shape)
        q = self.T1(q,k,v)
        q = self.T2(q,k,v)
        q = self.T3(q,k,v)
        return q
...
class Tail(nn.Module):
    def __init__(self, num_classes , num_frames, head=16):
        ...
        self.Qpr = nn.Conv2d(self.num_features, self.d_model, kernel_size=(13, 13), stride=1, padding=0, bias=False)
    ...
    def forward(self, x, b , t ):
        ...
        print("Tail input to bn2 ", x.shape)
        x = self.bn2(x)
        # stabilization
        ...
        print("Tail input to attention q", q.shape)
        print("Tail input to attention k", k.shape)
        print("Tail input to attention v", v.shape)
        outputs = []
        for i in range(self.head):
        ...
        if not self.training:
            return f
        y = self.classifier(f)
        #print("Tail output f", f)
        print("Tail output f", f.shape)
        #print("Tail output y", y)
        print("Tail output y", y.shape)
        #print("Tail output y", sum(y[0]))
...
class Semi_Transformer(nn.Module):
    ...
    def forward(self, x):
        b = x.size(0)
        t = x.size(1)
        print("shape of b to tail ", b)
        print("shape of  to tail ", t)
        
        x = x.view(b*t, x.size(2), x.size(3), x.size(4))
        x = self.base(x)
        # x: (b,t,2048,7,4)
        print("shape of input to tail ", x.shape)
